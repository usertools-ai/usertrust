/**
 * End-to-end integration test for usertrust SDK.
 *
 ...
 * correctly through the trust() entry point.
 */
...
describe("trust() — end-to-end integration", () => {
	let tmpVault: string;
	...
	describe("4. Policy denial blocks LLM call", () => {
		it("throws PolicyDeniedError and never calls the LLM", async () => {
			...
			const governed2 = await trust(allowedClient, {
				dryRun: true,
				vaultBase: tmpVault,
			});
	...
	describe("8. Destroy lifecycle", () => {
		...
		it("releases the audit lock so a new writer can acquire it", async () => {
			// First client
			const governed1 = await trust(makeAnthropicMock(), {
				dryRun: true,
				budget: 50_000,
				vaultBase: tmpVault,
			});
			...
			const governed2 = await trust(makeAnthropicMock(), {
				dryRun: true,
				budget: 50_000,
				vaultBase: tmpVault,
			});
	...
	describe("9. Config loading from vault", () => {
		it("loads budget from usertrust.config.json", async () => {
	...
	describe("10. Streaming support — wrapStream unit integration", () => {
		// Note: trust() does not natively intercept stream: true at the proxy
	...
	describe("11. Cross-provider isolation", () => {
		it("governs Anthropic and OpenAI clients independently", async () => {
			...
			const governedAnthropic = await trust(anthropicClient, {
				dryRun: true,
				budget: 50_000,
				vaultBase: tmpVault,
			});
			...
			const governedOpenAI = await trust(openaiClient, {
				dryRun: true,
				budget: 30_000,
				vaultBase: tmpVault2,
			});
