...
function makeTmpVault(): string {
	const dir = join(tmpdir(), `govern-dryrun-test-${randomUUID()}`);
...
describe("GOVERN_DRY_RUN=true", () => {
	...
	it("GOVERN_DRY_RUN env var activates dry-run mode", async () => {
		const mockAudit = makeMockAudit();
		const mockClient = makeAnthropicMock();

		vi.stubEnv("GOVERN_DRY_RUN", "true");
