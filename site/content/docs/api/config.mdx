...
**Not implemented.** Remote proxy mode is removed (AUD-456). Passing `proxy` to `trust()` or `createGovernor()` throws. Use a local TigerBeetle (or `dryRun`) for enforcement. These keys remain on the schema so existing config files still parse; they do not enable a hosted path.
...
| `feedProxy` | `false` | Unused. Reserved for a removed remote-proxy aggregation path; has no effect. |
