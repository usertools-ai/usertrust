...
interface TrustOpts {
  ...
  proxy?: string;         // Deprecated — not implemented; setting this throws (AUD-456)
  key?: string;           // Deprecated — proxy API key; unused because proxy throws
...
| `proxy` | `undefined` | **Not implemented.** Setting this option throws (AUD-456). Use a local TigerBeetle or `dryRun` for enforcement. |
| `key` | `undefined` | **Not implemented.** API key for the removed proxy. Unused; `proxy` throws first. |
